...
interface IMerkleDistributor {
    ...
    function changeMerkleRoot(bytes32) external returns(uint256);
...
contract MerkleDistributor is IMerkleDistributor {
    ...
    function pause() public override isUnLock() isOwner(){
        lock = true;
        emit Pause(round);
    }

    function start() public override isLock() isOwner(){
        lock = false;
        emit Start(round);
    }

    function changeMerkleRoot(bytes32 _merkleRoot) public override isLock() isOwner() returns(uint256){
