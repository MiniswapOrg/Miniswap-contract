...
contract MiniswapV2Pair is IMiniswapV2Pair, MiniswapV2ERC20 {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IMiniswapV2Factory(factory).feeTo();
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) override external lock {
        ...
        { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
            uint fee0 = amount0In.mul(3);
            uint fee1 = amount1In.mul(3);
            if(msg.sender == miner){
                fee0 = 0;
                fee1 = 0;
            } else {
                if(fee0 >0){
                    IMiniswapMiner(miner).mining(factory,to,token0,fee0.div(1000));
                }
                if(fee1 >0){
                    IMiniswapMiner(miner).mining(factory,to,token1,fee1.div(1000));
                }
            }
            uint balance0Adjusted = balance0.mul(1000).sub(fee0);
            uint balance1Adjusted = balance1.mul(1000).sub(fee1);
